...
NB_MODULE(tesseract_environment, m) {
    // Import dependent extension modules so their type casters are registered first.
    // Stub generation (nanobind_add_stub) loads the binding in isolation, so without
    // these explicit imports it fails with std::bad_cast when introspecting types that
    // reference symbols from other binding modules.
    tesseract_nb::import_modules_in_context(
        m,
        {
            "tesseract_common",
            "tesseract_geometry",
