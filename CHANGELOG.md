...
## [Unreleased] — TrajOptIfopt tuning + coordinated Descartes helpers + planning fixes

- **Coordinated Descartes helpers** — `RobotConfig`, `getRobotConfig`, and `getJointTurns` expose robot-configuration classification and redundant-turn detection in `tesseract_motion_planners`. Together with `sampleToolZAxis` / `sampleToolXAxis` in `tesseract_motion_planners_descartes` and `satisfiesLimits` in `tesseract_common`, these let custom Python waypoint samplers and state/edge evaluators check configurations and joint limits for robot-on-positioner and robot-with-external-positioner groups. Matching type stubs are included ([b07ba72f]).
...
- **TrajOpt factory collision margins** — `create_trajopt_default_profiles()` and `create_freespace_pipeline_profiles()` carried C++ `car_seat_example.cpp`'s collision buffers and coefficients but not its margins, so `default_margin` stayed `None` and the collision cost held nothing off obstacles: over OMPL seeds 1-100 the freespace example passed a median 0.17 mm from its sphere and failed the pipeline's contact check on 3. The constraint now gets C++'s 0 m margin and the cost its 5 mm: closest pass 4.98 mm, no failures ([#103], [#135]).
- **Low-level TrajOptIfopt example parameterizes time with ISP** — TOTG failed 14 of 200 OMPL seeds on TrajOptIfopt's trajectories ("Negative path velocity"); ISP, which tesseract's own `TrajOptIfoptTask` runs, failed none ([#103], [#135]).
- **OMPL example tests hold for every seed** — the per-test seed reaches OMPL only on Linux and macOS (conda-forge's Windows `ompl.lib` is static), so it hid the two failures above rather than preventing them. The seeds that exposed them replay as tests, and a seed-free test holds the freespace path at least half the cost margin off its obstacle ([#103], [#135]).
...
[b07ba72f]: https://github.com/tesseract-robotics/tesseract_nanobind/commit/b07ba72f
